...
class DroneVideoDisplay(QtGui.QMainWindow):
    ...
    def __init__(self, adbClient):
        ...
        print "Initializing drone video display"

        rospy.Subscriber('text_data', String, self.PictureCallback)
    ...
    def RedrawCallback(self):
        if self.image is not None:
            ...
            if self.takePicture:
                #image_message = self.image
                #cv_image = bridge.imgmsg_to_cv(image_message, desired_encoding="passthrough")
                filename = self.makeFilename()
                cv.SaveImage(filename, self.cvimage)
                twtr.postMediaUpdate(filename + ' #harveymudd #roboticslab #profdodds', filename)
                self.takePicture = False
